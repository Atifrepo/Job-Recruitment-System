...
class ViewApplicants extends Component {
  constructor() {
    ...
    this.state = {
      id: 0,
      e_mail: [{ email: '', name: '',DegreeTitle: '',UniversityName: '',
      Division: '',ToDate: '',FromDate: '', EnrollmentNumber: '',GPA:'' } ]
,
    }
  }
  componentDidMount() {
    ...
    var speedeRef = rootRef.child('Job/' + 'Apply/');
...
// DeleteApplicants(row) {
// console.log('row', row);

// var indexValue = this.state.key[row]
//  firebase.database().ref('Job/' + 'Apply/' ).remove();
            ...
            {this.state.e_mail.map((row) => {
              return (
                <TableRow key={row.id}>
                  ...
                  {/* <TableCell> <Button onClick={(id) => this.DeleteApplicants(i)} >Delete Applicants</Button>  </TableCell> */}
                </TableRow>
              );
            })}
          </TableBody>
...
export default withStyles(styles)(ViewApplicants)
